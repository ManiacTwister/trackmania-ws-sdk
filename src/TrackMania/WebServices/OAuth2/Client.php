 ...
 */
abstract class Client extends \TrackMania\WebServices\HTTPClient
{
	/**
	 ...
	 * @var string
	 */
	const AUTHORIZATION_URL = 'https://ws.trackmania.com/oauth2/authorize/';
	/**
	 * Logout URL
	 * 
	 * @var string
	 */
	const LOGOUT_URL = 'https://ws.trackmania.com/oauth2/authorize/logout/';
	/**
	 ...
	 * 
	 * @see \TrackMania\WebServices\OAuth2\Client::logout()
	...
	{
		return sprintf('trackmania:%s:%s', $this->username, $name);
	}
	...
	protected function getAccessToken()
	{
		...
		{
			if(isset($_REQUEST['state']) && $this->getVariable('state') && $_REQUEST['state'] != $this->getVariable('state'))
			{
				throw new \TrackMania\WebServices\Exception('CSRF attack protection failed');
			}
			$code = $_REQUEST['code'];
			if($code)
			{
	 ...
	 * 
	 * @see \TrackMania\WebServices\HTTPClient::execute()
	 */
